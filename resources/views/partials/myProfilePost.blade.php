<div class="active-tab menu-content post-tab-menu">
    ...
    <div class="card mb-4 post-container">
        ...
        @else
        <a href="{{ route('profile', $post->user->id )}}"> <span>@</span>{{$post->user->username}}</a> <span
            class="text-muted">
        ...
        <div class="mt-2 font-weight-bold">
            ...
            <a href="{{ route('post', $post->id) }}">
                @if($post->image !== null)
                <img class="card-img-top card-img thumbnail mr-1 mb-1" height="35" src="{{ asset($post->image) }}"
                    alt="Post Image">
                {{$post->title}} </a>
            @else
            <span class="ml-4"> {{$post->title}} </span> </a>
